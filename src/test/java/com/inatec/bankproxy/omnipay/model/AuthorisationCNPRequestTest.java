 ...
 */
public class AuthorisationCNPRequestTest {
    ...
    public void authorisationRequestIsCorrect() throws ParseException {
        ...
        AuthorisationCNPRequest authorisationCNPRequest = new AuthorisationCNPRequest();
        authorisationCNPRequest.setPrimaryBitmap(primaryBitmap);
        authorisationCNPRequest.setPrimaryAccountNumber(primaryAccountNumber);
        authorisationCNPRequest.setProcessingCode(processingCode);
        authorisationCNPRequest.setTransactionAmount(Integer.valueOf(StringUtils.stripStart(transactionAmount, "0")));

        SimpleDateFormat sdf = new SimpleDateFormat("MMddHHmmss");
        authorisationCNPRequest.setTransmissionDateTime(sdf.parse(transmissionDateTime));

        authorisationCNPRequest.setSystemTraceNumber(systemTraceNumber);

        authorisationCNPRequest.setLocalTransactionDateTime(sdf.parse(localTransactionDate + localTransactionTime));

        sdf = new SimpleDateFormat("yyMM");
        authorisationCNPRequest.setCardExpiryDate(sdf.parse(cardExpiryDate));

        authorisationCNPRequest.setMerchantCategoryCode(merchantCategoryCode);
        authorisationCNPRequest.setCountryCode(countryCode);
        authorisationCNPRequest.setPointOfServiceEntryMode(pointOfServiceEntryMode);
        authorisationCNPRequest.setPosConditionCode(posConditionCode);
        authorisationCNPRequest.setAcquiringInstitutionCode(acquiringInstitutionCode);
        authorisationCNPRequest.setRetrievalReferenceNumber(retrievalReferenceNumber);
        authorisationCNPRequest.setTerminalID(terminalID);
        authorisationCNPRequest.setMerchantNumber(merchantNumber);
        authorisationCNPRequest.setCardAcceptorNameAndLocation(cardAcceptorNameAndLocation);
        authorisationCNPRequest.setCvc2(cvc2);
        authorisationCNPRequest.setTransactionCurrencyCode(transactionCurrencyCode);
        authorisationCNPRequest.setAdditionalData(additionalData);

        String testedRequest = authorisationCNPRequest.toString();
